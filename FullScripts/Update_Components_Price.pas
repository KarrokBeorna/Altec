const
  CREATE_REPRICE = 'INSERT INTO REPRICES (ID, NAME, CREATORID, APPROVED, NOTE, EFFECTDATE) VALUES (gen_id(GEN_REPRICES, 1), :NAME, :USERID, 0, :NOTE, :DATE) RETURNING ID';
  GET_LAST_REPRICEID = 'SELECT MAX(ID) as ID FROM REPRICES';
  FIND_ARTICULID = 'SELECT MAX(ARTICULID) as ARTICULID FROM VIRTARTICULES WHERE AR_ART = :AR_ART and AR_LONGNAME = :AR_LONGNAME';
  CREATE_REPRICE_ITEM = 'INSERT INTO REPRICES_ITEMS (ID, REPRICEID, ARTICULID, PRICE) VALUES (gen_id(GEN_REPRICES_ITEMS, 1), :REPRICEID, :ARTICULID, :PRICE)';
