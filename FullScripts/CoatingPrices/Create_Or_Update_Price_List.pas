    ...
    for i := 1 to sFile.count - 1 do begin // со второй строки, так как на первой заголовки
      ...
      if currStr <> '' then begin
        ...
        for z := 0 to ARTICULIDs.Count - 1 do begin
          ...
          ArtO := S.OpenObject(IowArticul, ARTICULID);
          ...
          try
            ...
            BaseValue.InnerPrice := INPRICE;
            BaseValue.OuterPrice := OUTPRICE;
            BaseValue.BothPrice := DOUBLEPRICE;
            BaseValue.Apply;
          except
            visible := false;
            RaiseException('Артикул "' + ART + '" (строка №' + intToStr(i + 1) + ') ошибка во время установки цен - прайс-лист не может быть создан/обновлён');
          end;
        end;
      end;
    end;
