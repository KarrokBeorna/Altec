{
  ...
  * печатать отчёт "Трудоёмкость" сразу по всем выделенным
  * заказам. Просто подаём в отчёт вместо одного RecordID целый список из ID заказов.
  *
...
begin
  ...
  reportName := 'Трудоемкость заказа';
  S := TStringStream.Create(Session.QueryValue(GET_REPORTBLOB, MakeDictionary(['NAME', reportName, 'DOCTYPE', 'IowOrder'])));
