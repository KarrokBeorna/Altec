{
  * Скрипт позволяет проставлять плановые сроки изготовления компонентов
  * производственного заказа.
  *
  * SetProductionDate:
  * 1) Выставляет "Планирование изготовления" - выполнено;
  * 2) Выставляет всем остальным этапам конкретные плановые даты, если рядом с
  * их панелями стоит галочка:
  *   2.1) Дата высчитывается только из расчёта будней (учёт праздников не настроен);
  * 3) Если панель была доступна и было введено некоторое значение в "Длительности",
  * то данное значение будет записано в базу, чтобы при последующих запусках не
  * приходилось заново вводить длительности;
  * 4) В зависимости от того, какие поля диспетчеризации были заполнены, проходимся
  * по всему списку диспетчеризации и приравниваем плановые даты из данного списка
  * плановым датам изготовления непосредственно изделий.
  *
  * cbClicked:
  * 1) На вход принимает чекбокс;
  * 2) В зависимости от его индекса, взятого из тэга делает поля "Сдвиг" и
  * "Длительность" доступными для редактирования или нет (чекбосы, как и поля,
  * занесены в один словарь).
  *
  * CreatePanel:
  * 1) На вход принимает индекс, сдвиг (для поля "Сдвиг"), длительность (для
  * поля "Длительность"), смещение от левого края окна, смещение от верха окна;
  * 2) Рисует панель, вставляет в неё чекбокс, поле "Сдвиг", поле "Длительность"
  * для первых элементов;
  * 3) Все эти элементы заносятся в словарь, а словарь добавляет в список словарей.
  *
  * disablePanel:
  * 1) Отключает возможность редактирования панели по индексу.
  *
  * Main:
  * 1) Берем значения длительностей из базы;
  * 2) Высчитывается сдвиг для "Гибки профиля" в зависимости от длительности подготовки,
  * если стоит галочка "Требуется подготовка";
  * 3) Создается основное окно с панелью для выбора даты;
  * 4) Создаются поля для заполнения плановых дат диспетчеризации;
  * 5) При необходимости блокируются для редактирования некоторые панели;
  * 6) Высчитываются сдвиги для зависимых элементов;
  * 7) Добавляется кнопка "ОК"
}
...
begin
  ...
  if TCheckBox(Sender).checked then begin
    dictList[ind].value['shift'].enabled := True;
    try
      dictList[ind].value['duration'].enabled := True;
    except
    end;
  end else begin
    dictList[ind].value['shift'].enabled := False;
    try
      dictList[ind].value['duration'].enabled := False;
    except
    end;
  end;
end;
